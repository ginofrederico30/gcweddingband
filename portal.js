...
/* ============================================
   NOMINATIM ADDRESS AUTOCOMPLETE (fallback)
   ============================================ */
let _nominatimDebounce = {};
let _nominatimDropdown = null;

function _removeAutocompleteDropdown() {
...
function attachNominatimAutocomplete(inputId) {
  ...
  input.addEventListener('input', function() {
    ...
    _nominatimDebounce[inputId] = setTimeout(async () => {
      ...
      } catch (e) { _removeAutocompleteDropdown(); }
    }, 380);
  });

  input.addEventListener('blur', () => setTimeout(_removeAutocompleteDropdown, 200));
  input.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') _removeAutocompleteDropdown();
  });
}

function _showNominatimDropdown(input, results) {
...
function renderClientContract(clientId) {
  ...
  if (contract.signedAt && contract.adminSignedAt) {
    ...
    attachPlacesAutocomplete('cc-venue',   ['establishment', 'geocode']);
    attachPlacesAutocomplete('cc-address', ['address']);
    // Nominatim as fallback when Google Maps key not configured
    attachNominatimAutocomplete('cc-venue');
    attachNominatimAutocomplete('cc-address');
  }
}
